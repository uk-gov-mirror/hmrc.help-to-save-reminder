...
class HtsReminderMongoRepository @Inject()(mongo: ReactiveMongoComponent)
  ...
  override def findHtsUsersToProcess(): Future[Option[List[HtsUserSchedule]]] = {
    ...
    val filterDate = Some(LocalDate.parse("2021-02-06")).getOrElse(LocalDate.now())
    Logger.info(s"time for HtsUsersToProcess $filterDate")
    val testResult = Try {
      proxyCollection
        .find(Json.obj("nextSendDate" -> Map("$lte" -> filterDate)), Option.empty[JsObject])
